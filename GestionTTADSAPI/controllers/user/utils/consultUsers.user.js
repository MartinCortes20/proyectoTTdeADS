...
const consultUsers = async (req = request, res = response) => {
    ...
    try {
        ...
        if (!puedeConsultarAlumnos && !puedeConsultarDocentes && !puedeConsultarTodos) {
            return res.status(403).json({ message: 'No tienes permisos válidos para consultar usuarios.' });
        }
        ...
        if (puedeConsultarAlumnos && boleta) {
            ...
            console.log("Permiso 5 detectado. Consultando docentes por clave_empleado...");
            query = `
            ...
            return res.status(400).json({ message: 'No se detectaron parámetros válidos para tu consulta, recuerda que solo puedes bucar por boleta o clave de empleado.' });
        }
